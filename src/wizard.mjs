...
function buildConfig(owner, repo, host, workingDirectory, pre, user, post, notify) {
    return {
        ...
        git: { ref: 'main', credential: 'host-installed' },
        targets: [{
            name: host.split('.')[0],
            host,
            user,
            ...
            workingDirectory,
            pre: ['git pull --ff-only', ...pre.filter(command => command !== 'git pull --ff-only')],
            post
        }],
...
export async function saveConfigurationFile(owner, repo, jsonConfig, fs = defaultFs, path = defaultPath, crypto = { encrypt, isEncryptionConfigured }) {
    ...
    const encryptedRoot = process.env.KNIT_CONFIG_REPO_PATH || '/opt/knit-configs';
