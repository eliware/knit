...
export function createConfigLoader({ fsModule = fs, configPath = process.env.KNIT_CONFIG_PATH || path.resolve('repos'), log = console } = {}) {
  ...
  return { async load(name) {
    ...
    try { const text = fsModule.readFileSync(selected, 'utf8'); const config = yaml.load(text); if (!Validator.validate({ config, log })) return old?.config || null; cache.set(name, { path: selected, stamp, config }); return config; }
