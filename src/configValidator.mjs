...
export function validate({ config, log = logger }) {
  const target = t => t && typeof t === 'object' && string(t.workingDirectory) && string(t.host) && string(t.user) && commands(t.pre || []) && commands(t.post || []) && (!('identity' in t) || string(t.identity)) && (!('knownHosts' in t) || string(t.knownHosts));
